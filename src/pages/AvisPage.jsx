    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-4">{t('reviews.notAuthenticated')}</h2>
          <p className="text-gray-dark mb-6">{t('reviews.notAuthenticatedDesc')}</p>
  ...
  return (
    <div className="min-h-screen py-12 bg-gray-light">
      <div className="container mx-auto px-4 max-w-4xl">
        <h1 className="text-3xl font-bold mb-8 text-center">{t('reviews.title')}</h1>
        ...
        {selectedCommande ? (
          ...
          <>
            {commandes.filter(cmd => !avisExistants[cmd.idCommande]).length === 0 ? (
              <div className="bg-white-pure rounded-xl p-12 text-center">
                <p className="text-gray-dark mb-4">
                  {t('reviews.noOrdersToReview')}
                </p>
                <Link to="/menu" className="btn-primary inline-block">{t('cart.discoverMenu')}</Link>
              </div>
            ) : (
              <div className="space-y-4">
                <h2 className="text-xl font-semibold mb-4">{t('reviews.toReview')}</h2>
            ...
            {Object.keys(avisExistants).length > 0 && (
              <div className="mt-8">
                <h2 className="text-xl font-semibold mb-4">{t('reviews.reviewed')}</h2>
                <div className="space-y-4">
                  {commandes.filter(cmd => avisExistants[cmd.idCommande]).map((cmd) => (
